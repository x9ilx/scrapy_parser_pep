...
def test_run_scrapy(monkeypatch, temp_dir):
    ...
    assert any(
        'pep_' in str(file) for file in output_files
...
def test_check_correct_output_files():
    with open(
        [file for file in output_files if 'pep' in str(file)][0],
        'r',
    ) as file:
    ...
    with open(
        [file for file in output_files if 'status_summary_' in str(file)][0],
        'r',
    ) as file:
        file_result = file.read()
