...
def test_pep_spider():
    assert inspect.isclass(PepSpider), '`PepSpider` должен быть классом.'
    assert issubclass(
        PepSpider, scrapy.Spider
    ), '`PepSpider` должен наследоваться от `scrapy.Spider`'


def test_pep_spider_attrs():
    assert PepSpider.name, 'Класс `PepSpider` должен иметь атрибут `name`.'
    assert (
        PepSpider.name == 'pep'
    ), 'Значением атрибута `name` класса `PepSpider` лучше задать `pep`'
    assert hasattr(
        PepSpider, 'start_urls'
